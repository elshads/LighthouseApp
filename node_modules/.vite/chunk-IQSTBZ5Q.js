...
Fragment.prototype.nodesBetween = function nodesBetween(from3, to, f, nodeStart, parent) {
  ...
  for (var i = 0, pos = 0; pos < to; i++) {
    ...
    if (end2 > from3 && f(child3, nodeStart + pos, parent, i) !== false && child3.content.size) {
      var start3 = pos + 1;
      child3.nodesBetween(Math.max(0, from3 - start3), Math.min(child3.content.size, to - start3), f, nodeStart + start3);
    }
    pos = end2;
  }
};
  ...
  for (var j = slice3.openStart; j >= 0; j--) {
    ...
    for (var i$1 = 0; i$1 < targetDepths.length; i$1++) {
      ...
      if (parent.canReplaceWith(index2, index2, insert.type, insert.marks)) {
        return this.replace($from.before(targetDepth), expand ? $to.after(targetDepth) : to, new Slice(closeFragment(slice3.content, 0, slice3.openStart, openDepth), openDepth, slice3.openEnd));
      }
    }
  }
...
function sinkListItem(itemType) {
  return function(state, dispatch2) {
    ...
    if (dispatch2) {
      ...
      var slice3 = new Slice(Fragment.from(itemType.create(null, Fragment.from(parent.type.create(null, inner)))), nestedBefore ? 3 : 1, 0);
      var before2 = range.start, after2 = range.end;
      dispatch2(state.tr.step(new ReplaceAroundStep(before2 - (nestedBefore ? 3 : 1), after2, before2, after2, slice3, 1, true)).scrollIntoView());
    }
    return true;
  };
}
...
editHandlers.drop = function(view, e) {
  ...
  if (slice3) {
    ...
    slice3 = parseFromClipboard(view, e.dataTransfer.getData(brokenClipboardAPI ? "Text" : "text/plain"), brokenClipboardAPI ? null : e.dataTransfer.getData("text/html"), false, $mouse);
  }
...
function generateText(doc2, extensions2, options) {
  ...
  return getText(contentNode, {
    blockSeparator,
    textSerializers: __spreadValues(__spreadValues({}, textSerializers), getTextSeralizersFromSchema(schema))
  });
}
...
function getDebugJSON(node4, startOffset = 0) {
  ...
  const marks2 = node4.marks.map((mark3) => {
    ...
    if (Object.keys(mark3.attrs).length) {
      output2.attrs = __spreadValues({}, mark3.attrs);
    }
    return output2;
  });
  const attrs = __spreadValues({}, node4.attrs);
  const output = {
...
//# sourceMappingURL=chunk-IQSTBZ5Q.js.map
