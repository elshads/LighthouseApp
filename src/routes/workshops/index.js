    ...
    try {
        if (data.user_id > 0) {
            ...
            if (data.registered > 0) {
                sql = new PQ({
                    text: 'delete from workshop_appuser wu where wu.workshop_id = $1 and wu.appuser_id = $2',
                    values: [data.workshop_id, data.user_id]
                });
            } else {
                sql = new PQ({
                    text: 'insert into workshop_appuser (workshop_id, appuser_id) values ($1, $2)',
                    values: [data.workshop_id, data.user_id]
                });
            }
            let rows = await db.query(sql).catch((err) => {
